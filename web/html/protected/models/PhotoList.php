...
//                order by upload_time desc limit {$this->offset},{$this->length}";
	...
	p2.vote as vote_number, p2.view as view_number, p2.score as score_number from photo p3,
		(select p0.user_id, p0.act_id, sum(vote_number) as vote, sum(view_number) as view, sum(vote_number) * 10 + sum(view_number)  as score from photo p0 where p0.act_id={$this->act_id} group by user_id, act_id order by score desc limit {$this->offset}, {$this->length}) as p2
