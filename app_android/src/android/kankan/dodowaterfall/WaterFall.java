...
import android.kankan.MainProfile;
import android.kankan.R;
import android.kankan.SlidingLayout;
import android.kankan.SlidingMenu;
import android.kankan.dodowaterfall.LazyScrollView.OnScrollListener;
import android.kankan.dodowaterfall.widget.FlowTag;
import android.kankan.dodowaterfall.widget.FlowView;
					...
					if (t > 2 * scroll_height) {// ??????????????????
						...
						for (int k = 0; k < column_count; k++) {
							...
							if (pin_mark[k].get(Math.min(bottomIndex[k] + 1,
									lineIndex[k])) <= t + 3 * scroll_height) {// ????????????????????????????????????t+3*????????????
		...
		handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1:
					...
					// Log.d("MainActivity",
					// String.format(
					// "????????????View??????:%d,ID???%d,columnIndex:%d,rowIndex:%d,filename:%s",
					...
					//????????????????????????
				case 2:
					FlowView v2 = (FlowView) msg.obj;
					v2.getFlowTag().getFlowId();
					//v2.Reload();
					//v2.Reload();
					//Log.e("reload", "LLLLLLLLLLL");
					//					v.setImageBitmap(((FlowView) msg.obj).bitmap);
					//					w = msg.arg1;
					//					h = msg.arg2;
					//					// Log.d("MainActivity",
					//					// String.format(
					...
					//					// .getColumnIndex(), v.getRowIndex(),
					//					// v.getFlowTag().getFileName()));
					//					f = v.getFlowTag().getFileName();
					//
					//					// ??????????????????
					//					columnIndex = GetMinValue(column_height);
					//
					//					v.setColumnIndex(columnIndex);
					//
					//					column_height[columnIndex] += h;
